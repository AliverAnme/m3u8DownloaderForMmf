...
class FeedVideoItem:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'FeedVideoItem':
        ...
        binding_token_info = BindingTokenInfo.from_dict(binding_token_data) if binding_token_data else None
...
class Feed:
    ...
    def from_api_response(cls, response: Dict[str, Any]) -> 'Feed':
        ...
        if isinstance(items_data, list):
            feed.items = [FeedVideoItem.from_dict(item_data) for item_data in items_data]
    ...
    def sort_by_created_at(self, reverse: bool = True) -> None:
        """按创建时间排序视频项目"""
        self.items.sort(key=lambda x: x.get_datetime_created() or datetime.min, reverse=reverse)