        ...
        with self.get_connection() as conn:
            ...
            try:
                ...
                ''', (
                    ...
                    1 if collection.is_subscribed else 0 if collection.is_subscribed is not None else None,
                    1 if collection.is_post_in_collection else 0 if collection.is_post_in_collection is not None else None,
                    1 if collection.is_contributor else 0 if collection.is_contributor is not None else None,
                    1 if collection.can_commit else 0 if collection.can_commit is not None else None,
        ...
        with self.get_connection() as conn:
            ...
            try:
                ...
                for tag in video.tags:
                    cursor.execute('INSERT OR REPLACE INTO video_tags (video_id, tag) VALUES (?, ?)', (video.id, tag))
    ...
    def save_collection_videos(self, collection_id: str, video_ids: List[str]):
        ...
        with self.get_connection() as conn:
            ...
            try:
                ...
                # 先删除旧的关联关系
                cursor.execute('DELETE FROM collection_videos WHERE collection_id = ?', (collection_id,))
...
def make_api_request(url: str, max_retries: int = 3, retry_delay: float = 1.0, backoff_factor: float = 2.0) -> Optional[Dict[str, Any]]:
    """
...
def extract_title_from_description(description: str) -> str:
    ...
    hashtag_pattern = r'#逆愛|#柴雞蛋|#郭城宇|#展軒|#姜小帅|#刘轩丞|#Revenged\s*Love|#BTS|#花絮|#合集'
    ...
    if data:
        try:
            ...
            if not title or title.strip() == '':
                ...
                info(f"视频ID: {video_id} 的标题可能不完整，尝试从description中提取更完整的标题")
                extracted_title = extract_title_from_description(description)
                if extracted_title and extracted_title != "未获取到标题信息" and len(extracted_title) > len(title):
                    data_copy['title'] = extracted_title
            ...
            if not url and uid:
                filtered_data['url'] = f"https://videodelivery.net/{uid}/manifest/video.m3u8"
                info(f"视频ID: {video_id} 的URL为空，使用UID合成URL: {filtered_data['url']}")
...
# 定时检查集合更新并下载新增视频的函数
def check_collection_updates(collection_url, db_manager, download_flag=False, interval=120, download_dir="./downloads"):
    try:
        ...
        if data:
            ...
            # 如果启用了下载功能，下载所有视频
            process_and_download_videos(collection.contents[:5], db_manager, download_dir)
        else:
            error("获取集合数据失败")
    except Exception as e:
    ...
    try:
        # 启动一个线程来显示实时倒计时
        countdown_thread = threading.Thread(target=show_live_countdown, args=(interval,))
        countdown_thread.daemon = True  # 设置为守护线程，这样主线程结束时它也会结束
        countdown_thread.start()
        
        # interval秒后再次执行
        timer = threading.Timer(interval, check_feed_updates, args=[feed_url, db_manager, download_flag, interval])
...
def check_feed_updates(feed_url, db_manager: SimpleDatabaseManager, download_flag=False, interval=120, download_dir="./downloads",num=5,local_json_file="./temp/feed.json"):
    try:
        ...
        if data:
            ...
            info(f"分页信息: total={feed.total}, page={feed.page}, size={feed.size}, pages={feed.pages}")
            ...
            # 如果启用了下载功能，下载所有视频
            process_and_download_videos(feed.filter_by_author_id('BhhLJPlVvjU')[:num], db_manager, download_dir)
        else:
            error("获取订阅数据失败")
    except Exception as e:
    ...
    try:
        ...
        # interval秒后再次执行
        timer = threading.Timer(interval, check_feed_updates, args=[feed_url, db_manager, download_flag, interval, download_dir, num])
        timer.daemon = True  # 设置为守护线程
        timer.start()
    except Exception as e:
        # 捕获线程创建过程中的异常
...
# 处理并下载视频的函数
def process_and_download_videos(videos: list[FeedVideoItem], db_manager: SimpleDatabaseManager, download_dir="./downloads"):
    # 创建存储JSON文件的目录
    ...
    for video in videos:
        try:
            ...
            if video_record:
                ...
                output_filename = f"{safe_title}_{safe_date}.{download_manager.config.OUTPUT_FORMAT}"
...
def update_all_videos(db_manager: SimpleDatabaseManager, download_dir="./downloads"):
    ...
    try:
        ...
        if feed:
            ...
            # 如果启用了下载功能，下载所有视频
            process_and_download_videos(feed.filter_by_author_id('BhhLJPlVvjU'), db_manager, download_dir)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--interval', type=int, default=120, help='检查更新的时间间隔（秒），默认为120秒（2分钟）')
    ...
    # 开始定时检查订阅更新
    check_feed_updates(feed_url, db_manager, not args.no_download, args.interval, "./downloads",num=10)
