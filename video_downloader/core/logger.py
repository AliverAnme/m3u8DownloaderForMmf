...
class LoggerManager:
    ...
    def _initialize(self):
        ...
        # 日志文件目录
        self.log_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "log")
    ...
    def set_file_level(self, name: str, level: int) -> None:
        ...
        if name in self.loggers:
            for handler in self.loggers[name].handlers:
                if isinstance(handler, TimedRotatingFileHandler) and not handler.baseFilename.endswith("_error.log"):
                    handler.setLevel(level)
                    break
...
if __name__ == "__main__":
    ...
    info(f"日志文件已生成在: {os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'log')}")